...
class ToxicityAnalyzer:
    ...
    def build_model(self):
        ...
        model = tf.keras.models.load_model(r"C:\Users\vicky\Desktop\SDP\chat app\chatapp\toxicity.h5")
        return model
    ...
    def analyze_user_toxicity(self, users, convo_list):
        ...
        profanity_list=["Arsehole","Asshat","Asshole","BBC","Bitch","Bloody","Blowjob","Bugger","Bullshit","Chicken shit","Clusterfuck","Cock","Cocksucker","Coonass","Cornhole","Cox–Zucker machine","Cracker","Cunt","Dafuq","Damn","Dick","Enshittification","Faggot","Feck","Fuck","Healslut","Motherfucker","Nigga","Nigger","Paki","Poof","Poofter","Prick","Pussy","Ratfucking","Retard","Russian warship, go fuck yourself","Shit","Shithouse","Shitter","Slut","Son of a bitch","Spic","Twat","Wanker","Whore"]
        sensitive_topics = ["nazi", "hitler"]
        ...
        for convo in convo_list:
            ...
            # Heuristic: Profanity detection
            for word in profanity_list:
                if word in input_text:
                    res = self.predict_toxicity(input_text)
                    res[0] += 0.1  # Increase toxicity score

            # Heuristic: Identifying All Caps
            all_caps_percentage = sum(1 for word in input_text.split() if word.isupper()) / len(input_text.split())
            if all_caps_percentage > 0.3:
                res[0] += 0.1  # Increase toxicity score

            # Heuristic: Excessive Punctuation
            if input_text.count('!') + input_text.count('?') > 5:
                res[3] += 0.1  # Increase threat score

            if any(len(set(word)) <= len(word) / 2 for word in input_text.split()):
                res[0] += 0.1  # Increase toxicity score

            if any(char in emoji.UNICODE_EMOJI for char in input_text):
                res[4] += 0.1  # Increase insult score
            
            if any(topic in input_text for topic in sensitive_topics):
                res[0] += 0.1  # Increase toxicity score
    ...
    def visualize_user_toxicity(self, user, userno, avg_toxic, avg_severe_toxic, avg_obscene, avg_threat, avg_insult,
                                 avg_identity_hate):
        trait = 0
        if avg_toxic > 0.5:
